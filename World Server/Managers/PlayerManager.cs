...
namespace World_Server.Managers
{
    public class PlayerManager
    {
        ...
        public static void Boot()
        {
            ...
            WorldDataRouter.AddHandler<CMSG_PLAYER_LOGIN>(WorldOpcodes.CMSG_PLAYER_LOGIN, OnPlayerLogin);
            ...
            WorldDataRouter.AddHandler<CMSG_NAME_QUERY>(WorldOpcodes.CMSG_NAME_QUERY, OnNameQueryPacket);
        }

        private static void OnPlayerLogin(WorldSession session, CMSG_PLAYER_LOGIN packet)
        {
            ...
            session.sendPacket(new SMSG_SET_REST_START());
            ...
            session.sendPacket(new SMSG_SERVER_MESSAGE(3, "Servidor Omega 3 Beta Alpha, se esta vendo esta mensagem e por que funcionou.Tudo pode não funcionar!fique calmo, estou em desenvolvimento!"));

            // Envia Cinematic ao Char
            if (session.Character.firsttime == false)
            {
                session.sendPacket(new SMSG_TRIGGER_CINEMATIC(Program.Database.GetCharStarter(session.Character.Race).Cinematic));
        ...
        public static void OnNameQueryPacket(WorldSession session, CMSG_NAME_QUERY packet)
        {
        ...
        private static void OnLogoutRequest(WorldSession session, PacketReader reader)
        {
            ...
            session.sendPacket(new SMSG_LOGOUT_RESPONSE());
        ...
        public static void update()
        {
            while (true)
            {
                foreach (KeyValuePair<WorldSession, DateTime> entry in logoutQueue.ToArray())
                {
                    ...
                    {
                        entry.Key.sendPacket(new SMSG_LOGOUT_COMPLETE());
