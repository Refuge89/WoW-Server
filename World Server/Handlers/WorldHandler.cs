...
namespace World_Server.Handlers
{
    ...
    class WorldHandler
    {
        ...
        internal static void OnPlayerLogin(WorldSession session, CmsgPlayerLogin packet)
        {
            ...
            session.sendPacket(ObjectHandler.CreateOwnCharacterUpdate(session.Character, out Entity));//, packet));
            ...
            ChatHandler.SendSytemMessage(session, $"You logged in to: {session.Character.Name} [{session.Character.Class}@{session.Character.Race}] {session.Character.Id}");
        }
